...
class _ValidationVideoFeatureCache:
    ...
    def get_or_encode(
        ...
        if not globally_cacheable:
            ...
            logger.warning(
                "Validation video feature cache bypassed collectively: "
                "local_cacheable=%s pixel_rows=%s grid_rows=%s",
                local_cacheable,
        ...
        merged_split_sizes = [
            size // (spatial_merge_size**2) for size in raw_split_sizes
        ]
        resolved_keys = [
            (str(cache_key), grid) for cache_key, grid in zip(cache_keys, grids)
        ]
        ...
        if any_rank_missing and missing:
            ...
            miss_pixels = torch.cat(
                [pixel_chunks[index] for index in miss_indices], dim=0
            )
            miss_grids = torch.stack(
                [video_grid_thw[index] for index in miss_indices], dim=0
            )
            fresh_main, fresh_deepstack = encode_fn(miss_pixels, miss_grids)
            if len(fresh_main) != len(miss_indices):
                raise RuntimeError(
                    "validation video feature encoder returned an unexpected media count"
                )
            ...
            for fresh_index, (key, _) in enumerate(missing.items()):
                ...
                entry = (
                    ...
                    tuple(
                        layer_splits[fresh_index].detach().clone()
                        for layer_splits in fresh_deepstack_splits
                    ),
                )
                available[key] = entry
        ...
        deepstack = [
            torch.cat([entry[1][layer] for entry in ordered_entries], dim=0)
            for layer in range(deepstack_layer_count)
        ]
...
class HFModel(BaseModel):
    ...
    def __init__(
        ...
        self._tao_validation_video_feature_target = None
    ...
    def _configure_validation_video_feature_cache(self):
        raw_capacity = os.environ.get("TAO_VALIDATION_VIDEO_FEATURE_CACHE_SIZE", "0")
        try:
            capacity = int(raw_capacity)
        except ValueError as exc:
            raise ValueError(
                "TAO_VALIDATION_VIDEO_FEATURE_CACHE_SIZE must be an integer"
            ) from exc
        if capacity < 0:
            raise ValueError(
                "TAO_VALIDATION_VIDEO_FEATURE_CACHE_SIZE must be non-negative"
            )
        if capacity == 0:
            return
        ...
        if target is None or not callable(original) or not spatial_merge_size:
            raise RuntimeError(
                "Qwen3-VL validation video feature cache could not resolve the native vision encoder"
            )
        ...
        self._tao_validation_video_feature_target = target
        logger.info(
    ...
    def forward(
        ...
        **kwargs,
        ...
        tao_video_cache_keys = kwargs.pop("tao_video_cache_keys", None)
        kwargs_filtered = {
        ...
        self._tao_validation_video_cache_active = cache_active
        self._tao_validation_video_cache_keys = tao_video_cache_keys
        try:
            out = self.model(
                input_ids=input_ids,
