...
try:
    ...
    print(
        "transformer version too old, please upgrade to latest version, qwen model is not supported"
    )
...
def generate(
    ...
    *,
    ...
    # noqa: F841
    num_input_ids = inputs["input_ids"].shape[1]  # noqa: F841
...
class Qwen2_5_VisionTransformerPretrainedModel(nn.Module):
    ...
    def __init__(self, config) -> None:
        ...
        self.dtype = (
            torch.bfloat16 if config.torch_dtype == "bfloat16" else torch.float32
        )
        ...
        self.blocks = nn.ModuleList(
            [
                Qwen2_5_VLVisionBlock(config, config.attn_implementation)
...
class Qwen2_5_VLRotaryEmbedding(nn.Module):
    ...
    def _dynamic_frequency_update(self, position_ids, device):
        raise NotImplementedError(
            "Dynamic frequency update is not implemented for Qwen2_5_VL"
        )
        # """
...
class Qwen2_5_VLSdpaAttention(Qwen2_5_VLAttention):
    ...
    def forward(
        ...
        if (
            attention_mask is not None and attention_mask.ndim == 4
        ...
        if cp_mesh is not None:
            key_states = DTensor.from_local(
                key_states, cp_mesh, [Shard(2)]
            ).full_tensor()
            value_states = DTensor.from_local(
                value_states, cp_mesh, [Shard(2)]
            ).full_tensor()
...
class Qwen2_5_VLDecoderLayer(nn.Module):
    def __init__(self, config: Qwen2_5_VLConfig, layer_idx: int):
        ...
        self.self_attn = QWEN2_5_VL_ATTENTION_CLASSES[config.attn_implementation](
            config, layer_idx
        )
...
class Qwen2_5_VLModel(nn.Module):
    ...
    def forward(
        ...
        for decoder_layer in self.layers:
            ...
            if self.gradient_checkpointing and self.training:
                raise NotImplementedError(
                    "Gradient checkpointing is not implemented here"
                )
...
class Qwen2_5_VLForConditionalGenerationSimple(nn.Module):
    ...
    def forward(
        ...
        if inputs_embeds is None:
            ...
            is_inputs_embeds_dtensor = isinstance(
                inputs_embeds, DTensor
            )  # This is True for TP>1, False for TP=1
    ...
    if input_ids is not None and (
        ...
        for i, input_ids in enumerate(total_input_ids):
            ...
            for _ in range(image_nums + video_nums):
                ...
                range_tensor = torch.arange(llm_grid_t, device=input_ids.device).view(
                    -1, 1
                )
                expanded_range = range_tensor.expand(-1, llm_grid_h * llm_grid_w)
                ...
                time_tensor = (
                    expanded_range
                    * second_per_grid_t
                    * model_config.vision_config.tokens_per_second
                )
