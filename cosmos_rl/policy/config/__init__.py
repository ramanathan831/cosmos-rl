...
class SFTDataConfig(BaseModel):
    ...
    dataset_cache_dir: Optional[str] = Field(
        default=None,
        description="User-supplied cache root. No host-specific default is inferred when set.",
    )
    dataset_cache_fingerprint: Optional[str] = Field(
        default=None,
        description="Deterministic dataset+model+processor cache key generated by preflight.",
    )
    validation_dataset_cache_fingerprint: Optional[str] = Field(
        default=None,
        description="Validation dataset+model+processor cache key; required for a prewarmed validation cache when it differs from training.",
    )
    require_complete_dataset_cache: bool = Field(
        default=False,
        description="Require a validated prewarm manifest and all cache entries before training.",
    )
...
class LoraConfig(BaseModel):
    ...
    bias: Literal["none", "all", "lora_only"] = Field(
        default="none", description="Bias training behavior, matching PEFT semantics"
    )
    adapter_dtype: Optional[Literal["float32", "float16", "bfloat16"]] = Field(
        default=None,
        description="Optional adapter parameter dtype; unset inherits the base module dtype",
    )
...
class PolicyConfig(BaseModel):
    ...
    qwen3_vl_patch_embed: Literal["auto", "linear", "conv3d"] = Field(
        default="auto",
        description="A100-safe Qwen3-VL patch projection selection",
    )
